...
function Synchroniser (server, host, port, conf, interactive) {
  ...
  this.sync = (to, chunkLen, askedCautious, nopeers, noShufflePeers) => co(function*() {
    ...
    try {
      ...
      const downloader = new P2PDownloader(localNumber, to, rCurrent.hash, shuffledPeers, watcher, logger, hashf, rawer, Peer);
            ...
            for (const block of blocks) {
              ...
              if (block.number != to) {
                ...
                const HEAD = yield indexer.quickCompleteGlobalScope(block, currConf, bindex, iindex, mindex, cindex, sindex, {
                  ...
                  getBlockBuggy: (number) => {
                    return Promise.resolve(allBlocks[Math.max(0, number - 1)]);
                  },
                  getBlockByBlockstampBuggy: (blockstamp) => {
                    return Promise.resolve(allBlocks[Math.max(0, parseInt(blockstamp) - 1)]);
                  }
                });
                bindex.push(HEAD);
                ...
                if (block.dividend
                  ...
                  iindex = iindex.concat(yield indexer.ruleIndexGenExclusionByMembership(HEAD, local_mindex, dal));
                  iindex = iindex.concat(yield indexer.ruleIndexGenExclusionByCertificatons(HEAD, local_cindex, conf, dal));
...
function P2PDownloader(localNumber, to, toHash, peers, watcher, logger, hashf, rawer, Peer) {
