...
module.exports = {
  ...
  pull: (conf, dao, logger) => co(function *() {
    ...
    const applyCoroutine = (peer, blocks) => co(function*() {
      ...
      if (blocks.length > 0) {
        logger && logger.debug("Applying from " + blocks[0].number);
        let isFork = localCurrent
            && !(blocks[0].previousHash == localCurrent.hash
            && blocks[0].number == localCurrent.number + 1);
        if (!isFork) {
          logger && logger.debug('Peer %s is on same blockchain', shortPubkey);
          ...
          logger && logger.debug('Peer %s has forked', shortPubkey);
    ...
    const downloadChuncks = (peer) => co(function*() {
      ...
      let res;
      do {
        logger && logger.debug("dl starts from " + currentChunckStart);
        if (blocksToApply.length > 0)
          logger && logger.debug("apply starts from " + blocksToApply[0].number);
        res = yield {
