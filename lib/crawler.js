...
function PeerCrawler(server, conf, logger) {
  ...
  this.startService = () => co(function*() {
    ...
    crawlPeersInterval = setInterval(()  => crawlPeersFifo.push(() => crawlPeers(server, conf)), 1000 * conf.avgGenTime * constants.SYNC_PEERS_INTERVAL);
...
function PeerTester(server, conf, logger) {
  ...
  this.startService = () => co(function*() {
    ...
    testPeerFifoInterval = setInterval(() => testPeerFifo.push(testPeers.bind(null, server, conf, !FIRST_CALL)), 1000 * constants.TEST_PEERS_INTERVAL);
